...
- **Shared-Chat-Bewusstsein:** Der Reader abonniert zusätzlich die drei EventSub-Topics `channel.shared_chat.begin`/`update`/`end` (v1, kein Scope nötig). Solange eine Twitch-Shared-Chat-Session („Stream Together“) läuft, zeigt das Chat-Overlay einen dezenten Hinweis (als Gast mit dem Session-Host, als Host mit der Kanal-Anzahl); bei Session-Ende/Stopp/Kanalwechsel verschwindet er. Der Owner-Gate-Schutz gegen Ko-Streamer-Badges bleibt davon unberührt (siehe Abschnitt 2). Contract-Tests frieren die Parser-Härtung gegen reale Shared-Chat-Payloads (`shared_chat`, `source_*`, undokumentierte Zukunftsfelder) ein; ein CI-Guard erzwingt `ignoreUnknownKeys = true` für alle Chat-Json-Instanzen (Rückfall-Schutz).
...
- **Multi-Plattform-Chat (Phase P2):** Der Chat-Overlay kann zusätzlich den Kick-Chat eines Kanals einblenden (Settings: Kanal-Slug + Toggle) — gelesen wird anonym über das Pusher-Protokoll (`wss://ws-us2.pusher.com` mit dem öffentlichen App-Key der Kick-Web-App; Kanal-Auflösung über die öffentliche `api/v2/channels`-API), gemergt mit dem Twitch-/YouTube-Chat im selben Overlay; Kick-Nachrichten tragen ein grünes K-Plattform-Badge. Contract-Tests frieren die Pusher-Frames ein (Handshake, Subscription `chatrooms.<id>.v2`, `ChatMessageEvent` mit User-Farbe/Badges, Ping/Pong am Fake-Socket).
