...
| `!start` / `!go-live` · `!stop` / `!end` · `!diag` / `!status` · `!ask <frage>` · `!testalert <follow\|sub\|raid>` | **nur der Streamer** (Broadcaster-Badge oder Allow-List `chat_bot_owner_logins`) | Stream starten/stoppen · Diagnose mit Empfehlungen · Frage an die **exklusive Owner-KI** (Fallback: normale Bot-KI) · Test-Alert fürs Chat-Overlay — nur während eines aktiven Streams; Viewer erhalten nur einen Hinweis |
