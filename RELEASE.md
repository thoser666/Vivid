...
> **CodeQL-Alert-Bestand (Stand 27.08.2026):** **1 Error** — `java/android/implicit-pendingintents` (Use of implicit PendingIntents, Behebung ausstehend, Priorität P1); **2 Warnings** — `java/field-masks-super-field` (Feldmaskierung in Superklasse, 2 Vorkommen); **7 Notes** — `java/local-variable-is-never-read` (6× unbenutzte lokale Variablen) + `java/android/backup-enabled` (Application Backup erlaubt). **6 Alerts fixed** — `actions/missing-workflow-permissions` (alle Workflows haben jetzt Least-Privilege-Permissions). Insgesamt: 10 offen (1 Error, 2 Warning, 7 Note), 6 fixed. Die Error-Severity-Alerts (insb. `implicit-pendingintents`) haben höchste Priorität für die Behebung.
...
> - **CodeQL (oben aktiviert):** Scan läuft sauber (`success`), 10 offene Alerts (1 Error: `implicit-pendingintents`, 2 Warning: `field-masks-super-field`, 7 Note: `local-variable-is-never-read`/`backup-enabled`), 6 fixed. **Default Setup deaktiviert** (blockierte SARIF-Uploads). Fehlerbehebung P1: `implicit-pendingintents`.
