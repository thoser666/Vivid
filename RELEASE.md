...
| `!start` / `!go-live` · `!stop` / `!end` · `!diag` / `!status` · `!ask <frage>` · `!fix` · `!testalert <follow\|sub\|gift\|resub\|raid>` · `!torch` · `!filter <name>` | **nur der Streamer** (Allow-List `chat_bot_owner_logins` oder Kanal-Inhaber — der Broadcaster-Badge zählt nur im konfigurierten Kanal, nicht für Ko-Streamer aus Shared-Chat-Sessions) | Stream starten/stoppen · Diagnose mit Empfehlungen · Auto-Fix · Frage an die **exklusive Owner-KI** (Fallback: normale Bot-KI) · Test-Alert fürs Chat-Overlay · Taschenlampe umschalten · Video-Filter wechseln — nur während eines aktiven Streams; Viewer erhalten nur einen Hinweis |
| `!ban <user>` · `!timeout <user> <minuten?>` · `!delete <anzahl?>` | **nur der Streamer** (Allow-List `chat_bot_owner_logins` oder Kanal-Inhaber — der Broadcaster-Badge zählt nur im konfigurierten Kanal, nicht für Ko-Streamer aus Shared-Chat-Sessions) | Chat-Moderation über die Helix-API — Scopes `moderator:manage:banned_users` / `moderator:manage:chat_messages`, der Bot muss Moderator im Kanal sein: verbannen, timeouten (Standard 5 Min), die letzten N vom Bot gesehenen Nachrichten löschen |
...
Der Job **`verify-reproducibility`** in `release-pipeline.yml` vergleicht nach dem (täglichen) nightly-Publish die veröffentlichten Artefakte automatisch mit einem frischen Build desselben Commits (liest versionName/versionCode aus dem Release, prüft die eingebettete Git-Revision, Hash-Vergleich). Er läuft nur auf Schedule/Manual (nicht bei jedem develop-Push, seit das Nightly einmal täglich gebaut wird). **Alle drei Artefakte sind deterministisch und werden verglichen:** `app-standard-release.apk` (standard-Flavor — Asset- und Rebuild-Pfade sind flavor-qualifiziert, seit die App die Flavors standard/foss hat; Vorfall 06.09.2026: der Job lud das nicht existierende `app-release.apk` und scheiterte an jedem Nightly), `mapping.txt` (ProGuard/R8) und `output-metadata.json` — die Nightly-Releases enthalten daher neben dem APK auch Mapping und Metadaten (für Deobfuskation und Reproduzierbarkeits-Check). Schlägt ein Vergleich fehl, wird der Workflow rot.
...
**AAB (Bundle):** Sollte die Release-Lane künftig zusätzlich zum APK ein `app-standard-playRelease.aab` publizieren, wird es automatisch mitgeprüft (der Download-Step erkennt das AAB im Release und aktiviert die AAB-Checks nur dann): ① **Signatur** per `jarsigner -verify` (Integrität) + `keytool -printcert -jarfile` (Zertifikat gegen Release-Key), ② **Reproduzierbarkeit** durch frischen `:app:bundleStandardPlayRelease`-Build mit denselben versionName/versionCode-Parametern und Hash-Vergleich. Die AAB-Determinismustests lokal bestätigen: Zwei vollständig frische `bundleStandardPlayRelease`-Builds desselben Commits sind **bit-identisch** (Vorsicht: ein mit `--rerun-tasks` gemischter Vergleich gegen einen zwischenzeitlich veralteten Task-Cache täuscht Unterschiede vor — nur frische Builds vergleichen).
...
**Retry-Härtung der Release-Builds (transiente Netz-/API-Fehler):** Release-grade Gradle-Builds (`assembleRelease` ×2 — `build_release`-Lane und `release_github`-Lane — und `bundleStandardPlayRelease` in `publish_play`) sind über den gemeinsamen Helper **`fastlane/build_retry.rb`** gehärtet (`BuildRetry.with_gradle_retry`): Der Sentry-Mapping-Upload läuft als Gradle-Task (`uploadSentryProguardMappings*`) **innerhalb** des Builds und ist die einzige netzwerkabhängige Stufe der sonst deterministischen Release-Builds. Vorfall 05.09.2026 (Beta `v0.5.12-beta`, Run 33952343595): `SSL connect error (Connection reset by peer)` nach 5:48 min Buildzeit — Run rot, ohne Release. Der Helper retryt solche transienten Fehler **3× mit linearem Backoff (10 s / 20 s)**; dank Gradle-Build-Cache kostet der zweite Versuch nur den fehlgeschlagenen Task, nicht die vollen Build-Minuten. Bewusst **ENG gefasst:** Nur Muster wie `SSL connect error`, `Connection reset by peer`, Timeouts, DNS-Fehler und der Sentry-Upload-Task gelten als transient — deterministische Fehler (Kompilierung, Keystore, Signatur) scheitern weiterhin sofort, nichts wird maskiert. Regressionstest: `scripts/test_build_retry.sh` (Klassifikation, Retry-Zähler, Backoff, statische Verdrahtungs-Guards) — im CI als Job **„Self-Test Build-Retry (Hardening)“** und im Pre-Push-Gate. Die `gh release create`-Aufrufe hatten bereits eigene Retries (siehe Rollback-Semantik unten).

**Retry-Härtung des Emulator-Gates (Umwelt-Flakiness):** Der Stable-Publish-Gate-Step („Run instrumented tests on emulator (release gate)“ in `distribution-stable.yml`) ist über den Wrapper **`scripts/emulator_gate_retry.sh`** gehärtet. Vorfall 24.09.2026 (Release `v0.5.19-beta`, Run 36025777687): `:app:connectedStandardDebugAndroidTest` schlug im Gate fehl („There were failing tests“) — bei exakt demselben Commit, dessen Suite 40 Minuten vorher im Tag-Push-Run (36021918352) auf derselben ubuntu-x86_64-Leg grün war; Emulator-auf-Hosted-Runner ist eine inhärent umweltabhängige Stufe. Der Wrapper retryt transiente Klassen **3× mit linearem Backoff (10 s / 20 s, wie `build_retry.rb`)**; deterministische Fehler (Kompilierung der Test-Quellen) haben **Vorrang** und scheitern sofort, unklassifizierte Fehler bleiben **fail-closed** — nichts wird maskiert. Evidenz-basierte Klassifikation: Boot-Noise, der auch in grünen Läufen steht („Unable to connect to adb daemon“, „Failed to load snapshot“, ColorBuffer-Warnungen), ist bewusst **kein** Retry-Muster; transient sind nur die Fehlerklassen am Log-Ende (Suite-Fehlschlag, Geräteverlust mid-run, Boot-Fehler, Dependency-Auflösung). Der komplette Output jedes Versuchs wird transparent ausgegeben. Regressionstest: `scripts/test_emulator_gate_retry.sh` (E1–E10, inkl. der Grün-Run-Noise-Fixture als Kernbeweis E1.3) — im CI als Job **„Self-Test Emulator-Gate-Retry (Hardening)“** und im Pre-Push-Gate.
...
**Roborazzi Screenshot-Tests (konfiguriert, inaktiv):** Roborazzi 1.72.0 ist konfiguriert (Plugin in `app/build.gradle.kts`, Dependencies in `libs.versions.toml`, Tests in `app/src/test/.../screenshot/ScreenshotTest.kt`), aber die Tests können aktuell nicht ausgeführt werden wegen Inkompatibilität mit AGP 9.3.2 + Robolectric 4.14.1 + JDK 25: `ComposeView` kann keinen `WindowRecomposer` finden ("not attached to a window"). Sobald Roborazzi AGP 9.x vollständig unterstützt, können die Tests reaktiviert werden (`./gradlew :app:recordRoborazziStandardDebug`). Visuelle Regression läuft aktuell über die bestehende Screengrab-Lane im Emulator.
...
> 🔬 **CodeQL (GitHub Security Scanning, aktiv seit 26.08.2026):** Der Workflow [.github/workflows/security-codeql.yml](.github/workflows/security-codeql.yml) analysiert die Kotlin/Java-Codebasis pro `push`/`pull_request` auf `master`/`develop`, **manuell per `workflow_dispatch`** sowie **wöchentlich (Mo 04:00 UTC)** im Default-Branch. Beide Actions sind auf den **immutable SHA von `v4.38.0`** gepinnt (`init`/`analyze` aus demselben Release-Tag; Commit `b96794f01…` inkl. `# v4.38.0`-Kommentar, konsistent zur übrigen SHA-Pinning-Linie). Zusätzlich zu den Standard-Queries: **`security-extended` + `security-and-quality`**. Least-Privilege: `contents: read` + `security-events: write` (nur der CodeQL-Scan schreibt seine Findings), `concurrency`-Gruppe `codeql-${{ github.ref }}` (abbrechen bei Re-Push). Der manuelle Build (`./gradlew :app:assembleDebug` nach JDK-25-Setup) ersetzt den Autobuild für deterministisches Tracing; Ergebnisse erscheinen in **Security → Code Scanning** (und im Commit-Status). **Kotlin-Extractor-Pin (seit 11.09.2026):** Da Bundle 2.27.0 Kotlin ≥ 2.4.20 ablehnt, pinnt ein Step vor dem Build den Trace auf Kotlin 2.4.10 (siehe Bundle-Lag-Lehre unten) — die normale CI baut unverändert 2.4.20, die Analyse deckt denselben Quellcode ab. Update-Fahrplan: SHA-Pins wie bei den übrigen Actions per `.github/dependabot.yml` (`package-ecosystem: github-actions`, weekly) — die `# v4.x.y`-Kommentare sind dabei Pflicht.
>
> ⚠️ **Bundle-Lag-Lehre (10.09.2026, empirisch verifiziert):** Die Action-Version (SHA-gepinnt) und der Default-**Bundle**-Stand sind **zwei getrennte Versionen** — und das Release-Timing der beiden ist **nicht synchron zum Extractor-Support**: Nach dem Kotlin-2.4.20-Bump lehnte Bundle **2.26.4** die GA-Version ab („Kotlin version 2.4.20 is too recent“, Tracking github/codeql#22404). Der Wächter wurde daraufhin entfernt (b2d92ed, „Issue geschlossen = unterstützt“) — **zu früh, wie der empirische Test zeigte**: Auch Bundle **2.27.0** (Default von codeql-action **v4.38.0**, released 09.09.2026) lehnt Kotlin 2.4.20 weiterhin ab — der Extractor-Support (PR github/codeql#20018) ist upstream gemerged, aber in **keinem** released Bundle ausgeliefert (Nachweis: CodeQL-Run `34509237841` auf Bundle 2.27.0, identischer Fehler). **Status: Blockade aufgehoben (11.09.2026, Workaround):** security-codeql.yml pinnt den Trace-Build auf **Kotlin 2.4.10** (letzte Version < 2.4.20, bewährtes Pair mit KSP 2.3.11; Pin-Step „Pin Kotlin for CodeQL extractor", Quelle dynamisch aus dem Versions-Katalog, Reihenfolge vor dem Build — Tests: `scripts/test_codeql_kotlin.sh` K1–K6). Damit ist **CodeQL wieder grün**, ohne den Normal-CI-Kotlin zu berühren; die v4.38.0-Pins bleiben trotzdem (aktuellster Bundle-Stand + behebt die GitHub-Warnung „Not all workflow steps that use github/codeql-action use the same version“). **Revert:** Sobald ein Bundle > 2.27.0 den 2.4.20-Extractor ausliefert, Pin-Step entfernen — der advisory Wächter `scripts/check_codeql_blockade.sh` (im Pre-Push-Gate, nie blockierend) sagt genau das und warnt automatisch. **Für künftige Kotlin-Bumps gilt:** Issue geschlossen ≠ im Bundle ausgeliefert — erst der grüne CodeQL-Lauf zählt.
...
> - **CodeQL (oben aktiviert):** Scan grün — der Trace-Build läuft dank Kotlin-Extractor-Pin (2.4.10, siehe Bundle-Lag-Lehre) wieder durch; **0 offene Alerts** — 10 False Positives dismissiert (Kotlin-Compiler-Artefakte + bewusste Konfiguration), 6 fixed, 2x `override val message` präventiv. **Default Setup deaktiviert** (blockierte SARIF-Uploads). CodeQL-Status: ✅ sauber.
...
Der direkte Dependabot-Alert `kotlin-gradle-plugin` (unsafe Deserialization im Kotlin Build Cache, Dependabot #63) ist **geschlossen**: `kotlin` und `jetbrainsKotlinJvm` wurden am 10.09.2026 auf **2.4.20** (stabil, seit 07.09. auf Maven Central) angehoben. Blocker `github/codeql#22404` („Kotlin version 2.4.20 is too recent") wurde am **08.09.2026 im Issue-Portal geschlossen**, der Extractor ist aber in **keinem released CodeQL-Bundle** ausgeliefert (Stand Bundle **2.27.0**, 11.09.2026) — siehe „Bundle-Lag-Lehre" oben: `security-codeql.yml` pinnt den Trace-Build auf Kotlin 2.4.10. Der frühere Revert (`e5cd592` → `a8766e5`) ist damit aufgehoben, die Pipeline baut 2.4.20.
...
- Der CodeQL-Kotlin-Wächter (`check_codeql_kotlin_support.sh` + `automation-codeql-kotlin.yml`) wurde am **10.09.2026 entfernt** — die Annahme „Blocker codeql#22404 geschlossen = native Unterstützung" war **zu optimistisch**: im released Bundle steht die 2.4.20-Extraktion bis heute aus. Als Ersatz eingebaut (11.09.2026): der **Extractor-Pin-Step** in `security-codeql.yml` (Trace auf Kotlin 2.4.10) + Regressionstest `scripts/test_codeql_kotlin.sh`; die advisory Blockade-Frühwarnung läuft über `scripts/check_codeql_blockade.sh`. Die eigentliche CodeQL-Analyse läuft weiter über `security-codeql.yml`.
