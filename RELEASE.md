...
> **CodeQL-Alert-Bestand (Stand 28.08.2026):** **0 Errors** (1 False Positive: `implicit-pendingintents` dismissiert — Code nutzt explizite Intents + `FLAG_IMMUTABLE`, siehe github.com/github/codeql/issues/20153); **0 Warnings** (2x `field-masks-super-field` durch `override val message` behoben); **7 Notes** — `java/local-variable-is-never-read` (6× unbenutzte lokale Variablen) + `java/android/backup-enabled` (Application Backup erlaubt). **8 Alerts fixed** — 6x `actions/missing-workflow-permissions` + 2x `field-masks-super-field`. Insgesamt: 7 offen (0 Error, 0 Warning, 7 Note), 8 fixed, 1 False Positive.
...
> - **CodeQL (oben aktiviert):** Scan läuft sauber (`success`), 7 offene Alerts (0 Error: `implicit-pendingintents` als False Positive dismissiert, 0 Warning: `field-masks-super-field` durch `override val message` behoben, 7 Note: `local-variable-is-never-read`/`backup-enabled`), 8 fixed, 1 False Positive. **Default Setup deaktiviert** (blockierte SARIF-Uploads). False Positive: Code nutzt explizite Intents + `FLAG_IMMUTABLE`.
