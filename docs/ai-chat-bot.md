...
| Diagnose | `!diag` / `!status` | **Diagnose-Lauf:** sammelt deterministisch Stream-Status (inkl. Fehlerursache), OBS-Verbindung und Konfigurations-Checks (URL/Key, Multi-Streaming, Chat-Kanal, Bot-Token, **Whisper-Antwortweg (Client-ID + Token)**, Viewer-/Owner-LLM, **Owner-KI-Quelle** — offen, wenn weder Owner- noch Viewer-LLM konfiguriert sind, **Event-Alerts konfiguriert** — Kanal/Bot-Login/Token/Client-ID gesetzt; Scopes/Mod-Rechte verifiziert Twitch nicht im Check, fehlende Rechte lassen nur den jeweiligen Alert-Typ ausfallen; **Crash-Zusammenfassung** — zählt die markierten Abstürze aus dem LogStore innerhalb der Vorhaltezeit („X Crashes in Y Tagen“, ok bei keinen Crashes) und verweist zur Auswertung auf den Log-Screen). **Owner-KI exklusiv** → Bewertung + konkrete Empfehlungen; ohne eigene Owner-KI → **Viewer-KI als Fallback**; ohne jede KI → Checkliste direkt im Chat. Die Antwort weist die **KI-Quelle** aus („Auswertung durch: eigene Owner-KI / Viewer-KI (Fallback) / deterministisch“) |
...
| Test-Alert | `!testalert <follow\|sub\|gift\|resub\|raid\|hype>` | Löst über die **Trigger-API** einen synthetischen Event-Alert im Chat-Overlay aus (erscheint dort sofort, wenn der Streaming-Screen offen ist) — zum Prüfen des Overlay-Renderings vor dem Go-Live, ohne echte Follows/Subs/Raids abzuwarten. `hype` simuliert einen Hype-Train-Banner (bleibt bis zum Deaktivieren/`end` stehen, im Gegensatz zu den 10-s-Alerts). Fehlt/ungültig der Typ, antwortet der Bot mit `Nutzung: !testalert follow\|sub\|gift\|resub\|raid\|hype`; kein Scope nötig |
...
| Video-Filter | `!filter <name>` (Alias `!fx`) | Wechselt den aktiven OpenGL-Video-Effekt (Graustufen, Sepia, Rauschen, Negativ, Kanten, Cartoon, Pixeliert, Unschärfe, Schönheit, Duotone oder Kein Filter). Ohne Argument → nächster Filter (zirkulär); mit Name → spezifischer Filter; Antwort zeigt die verfügbaren Filter |
| Low-Light-Boost | `!boost` (Alias `!lowlight`/`!low-light`) | Schaltet den software-basierten Low-Light-Boost (1.5x Helligkeitsanhebung via OpenGL-Brightness-Filter) um — funktioniert auf allen Videoquellen (Kamera, Screen-Capture, Video-Player). Bestätigung „☀️ Low-Light-Boost AN / 🌙 Low-Light-Boost AUS.“ über den privaten Antwortweg; kein Scope nötig |
| Akku-Anzeige | `!battery` (Alias `!akku`) | Zeigt den aktuellen Batteriestand als Chat-Nachricht (z. B. „🔋 Akkustand: 75%"). Bei niedrigem Akku (unter konfigurierbarem Schwellenwert) sendet der Bot alle 10 Minuten eine Warnung „⚠️ Akku niedrig (X%) — bitte laden!“. Schwellenwert: `batteryLowThresholdPercent` in den Einstellungen (5–50%, Standard 15%) |
| 3D-LUT | `!lut <warm\|cool\|none>` | Wechselt den 3D-LUT-Preset (Farbton-Transformation via Hald-CLUT GLSL-Shader). Ohne Argument → nächster Preset (zirkulär: NONE → WARM → COOL → NONE); mit Name → spezifischer Preset. Warm = warmer Ambeer-Ton, Cool = kühler Blau-Ton, None = Passthrough. Antwort „🎬 3D-LUT: Warm/Cool/None“ |
| Color-Space | `!colorspace <srgb\|p3\|log>` (Alias `!cs`) | Wechselt den Gamma-Korrektur-Modus für den 3D-LUT-Filter. sRGB = Standard-Gamma (2.2), Display P3 = breiterer Farbraum (2.2), Apple Log = logarithmisches Encodierung (1.0). Antwort „🖥️ Color-Space: sRGB/Display P3/Apple Log“ |
...
- **PREFIX-Scope:** präfixierte Formen `!v!start` / `!v!stop` / `!v!diag` / `!v!ask <frage>` · `!v!testalert <follow\|sub\|raid\|hype>` · `!v!torch` · `!v!filter` · `!v!boost` · `!v!battery` · `!v!lut` · `!v!colorspace` / `!v!cs` · `!v!ban <user>` / `!v!timeout <user> <minuten?>` / `!v!delete <anzahl?>` (Präfix `v`) — die generischen `!`-Formen gehören dann dem anderen Bot (Koexistenz-Modus).
...
- `TwitchChatEventSubReader` — EventSub-WebSocket-Reader für `channel.chat.message` (Scope `user:read:chat`): `session_welcome` → Helix-Subscribe, `notification` → `ChatMessage` (Badges/Emotes/Farbe/Zeitstempel), `session_reconnect` + Backoff-Reconnect. Ersetzt den IRC-Leser für Overlay **und** Bot. Alle Json-Instanzen sind mit `ignoreUnknownKeys = true` gehärtet (Twitch erweitert die Payloads laufend — `shared_chat`, `source_*`); ein CI-Guard (`scripts/check_eventsub_json_hardening.sh`) verhindert Rückfälle, Contract-Tests mit realen Shared-Chat-Payloads frieren das Verhalten ein.
