...
- **BinaryArtifacts (#34–#40):** F-Droid-Index-JARs (`docs/fdroid/{repo,archive}`) und der Gradle-Wrapper sind erforderliche, absichtlich versionierte Artefakte. **Kontextiert (03.09.2026):** `.github/scorecard.yml` mit Maintainer-Annotations — F-Droid-JARs `not-applicable` (signierte Distributions-Indizes an clientfixen URLs) + `remediated` (reproduzieller PR-basierter Rebuild, Client-seitige Signaturprüfung), Wrapper-JAR `not-supported` (Checksummen-DB-Validierung in CI, von Scorecard nicht erkennbar), Fuzzing `not-applicable` (keine Scorecard-erkannte Fuzzer-Kategorie für Kotlin/Android). Zudem läuft der CI-Job „Build & Test“ jetzt vor jedem Gradle-Aufruf `gradle/actions/wrapper-validation` (SHA-gepinnt auf `9c971963…`, v6.3.0): prüft `gradle-wrapper.jar` gegen die offizielle Gradle-Wrapper-Checksummen-DB und schlägt bei unbekannten JARs fehl. Beides ist durch `scripts/test_workflow_security.sh` abgesichert (Annotations-Vokabular, SHA-Pin, Reihenfolge vor erstem `./gradlew`).
...
- **Fuzzing (#43), CIIBestPractices (#45):** Prozess-/Einrichtungsthemen, keine Quellcode-Änderungen. Fuzzing ist zusätzlich per Maintainer-Annotation (`.github/scorecard.yml`, `not-applicable`) kontextiert — Scorecard erkennt keine Android/JVM-Fuzzer-Integration. Für das OpenSSF CII Best Practices Badge (Grundlage von #45) gibt es eine vollständige Kriterien-Checkliste in [`docs/cii-best-practices-badge.md`](docs/cii-best-practices-badge.md).
