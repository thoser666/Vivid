...
| Datum | Änderung |
|---|---|
| 2026-09-13 | **Re-Scan-Verifikation:** `#471 backup-enabled`, `#473 local-variable-is-never-read` und der **ursprüngliche `#474 deprecated-call`** wurden per Code-Fix analyseseitig als `fixed` geschlossen (der async-`GeocodeListener`-Pfad ab API 33 greift; die Query akzeptiert den SDK-Guard). Für den verbleibenden minSdk-24-Sync-Fallback erzeugte der Scan die **neue Instanz #476** (Zeile 75) — diese wurde per GitHub-API als `won't fix` dismissed (Register-Zeile #474 → #476 umgetitelt). `#472` persistierte trotz Source-Refactor (`!is`-Guard) als Kotlin-Temp-Finding (Dateiebene) und wurde am selben Tag als `false positive` gedismissed. SonarCloud-Hinweis ergänzt: `xml:S5332`-Hotspot ist nur per **manueller UI-Aktion** als SAFE reviewbar (kein Token-/API-Dismissal). |
