...
- 💬 **Twitch Chat Overlay** - Show the chat of any Twitch channel over the live preview (read via Twitch EventSub, no IRC). Configure the channel and toggle the overlay in Settings → **Chat-Overlay**; the latest messages appear bottom-left with each user's Twitch color and **inline Twitch emotes** (rendered as CDN images via Coil), the connection auto-reconnects, and the overlay hides itself as soon as it is disabled in the settings
...
| UI Color Schemes (Vivid extra) | 🚧 | Material-3 palette (seed `#3DDC84`) replaces the template colors; Dark/Light follows the system (`isSystemInDarkTheme()`); Stufe 2 planned: user toggle (System/Light/Dark/AMOLED) + accent color as „Darstellung“ settings category |
