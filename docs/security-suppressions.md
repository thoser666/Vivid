...
| GitHub Dependabot | Alert-Dismissal (`tolerable_risk`) | 4 (#14/#16/#26/#27 — davon 1 obsolet: #26, Graph ≥ Fix) — 5 der vormals 9 wurden durch Fixes geschlossen (#19/#20 via BC-Buildscript-Pin 1.85 am 20.09., #62/#64/#65 via Networking-Group-Bump #185 am 21.09.) | Einzel-Review 2026-09-11 (Ist-Versionen + Scope-Nachweis im Register) + monatlicher Review-Workflow |
...
Dismissed am 2026-08-15 durch **thoser666**, einheitlich `tolerable_risk`. **Einzel-Review 2026-09-11** (verwundbare Range vs. tatsächlich aufgelöster Version in `settings.gradle.kts`-Submission-Grafik + Runtime-Classpath-Scan beider Flavors): **#26 ist obsolet** (Graph löst 4.5.14 ≥ Fix 4.5.13 — Alert müsste bei nächster Submission automatisch auf `fixed` gehen), die übrigen 8 waren (Stand 11.09.) in der verwundbaren Range, aber durch den harten Scope-Nachweis entschärft: **keines der 9 Pakete liegt im App-Runtime-Classpath** (`:app:dependencies` standard+foss, je 1.481 Zeilen, 0 Treffer) — sie leben ausschließlich im Build-/Submission-Kontext (AGP-Tooling: jetifier, manifest-merger; Sonar-Scanner-JAR; Ktor-3.x-Buildzeitlinie). **Update 2026-09-25 (Live-Abgleich):** Fünf der neun Dismissals wurden durch Fixes geschlossen — #19/#20 am 20.09. via BC-Buildscript-Pin 1.85 (`892451c`), #62/#64/#65 am 21.09. via Networking-Group-Bump #185; dismissed verbleiben #14/#16/#26/#27. Review bis **2027-03-11**:
...
| #19 | Riskante Kryptografie in bcpkix | `org.bouncycastle:bcpkix-jdk18on` | >= 1.49, < 1.84 | **1.80.2** | ✅ **fixed** (2026-09-20): BC-Buildscript-Pin 1.85 (`892451c`) hob die Submission-Grafik auf 1.85 — Alert automatisch geschlossen | Historische Begründung (11.09.): Build-/Submission-Kontext; bcpkix-Modul nicht für Signatur-/Zertifikatspfade der App verwendet, verwundbarer Codepfad im Shipped-Build nicht erreichbar. |
| #20 | LDAP Injection in bcprov | `org.bouncycastle:bcprov-jdk18on` | >= 1.74, < 1.84 | **1.80.2** | ✅ **fixed** (2026-09-20): BC-Buildscript-Pin 1.85 (`892451c`) — der angekündigte Fix-Weg ist vollzogen | Historische Begründung (11.09.): Build-/Submission-Kontext; App nutzt keine LDAP-Verbindungen. |
...
| #62 | HTTP/1 Header-Parsing-Memory-Exhaustion | `org.apache.httpcomponents.core5:httpcore5` | < 5.4.3 | **5.3.6** (Cache-Beleg) | ✅ **fixed** (2026-09-21, Networking-Group-Bump #185: httpcore5 ≥ 5.4.3) | Historische Begründung (11.09.): Ktor-3.x-Buildzeitlinie (ktor-client-apache5, nicht im Vivid-Graph); DoS-Vektor adressiert Server-Parsing — im App-Runtime nicht vorhanden. |
| #64 | HPackDecoder unbegrenzte Header-Liste | `org.apache.httpcomponents.core5:httpcore5-h2` | < 5.4.3 | **5.3.6** (Cache-Beleg) | ✅ **fixed** (2026-09-21, Networking-Group-Bump #185: httpcore5-h2 ≥ 5.4.3) | Historische Begründung (11.09.): Ktor-3.x-Buildzeitlinie; HTTP/2-Decode-Pfad wird in der App nicht als Server betrieben. |
| #65 | Connection Leak bei Content-Encoding-Fehler (DoS) | `org.apache.httpcomponents.client5:httpclient5` | >= 5.0-alpha1, < 5.6.3 | **5.5.1** (Cache-Beleg) | ✅ **fixed** (2026-09-21, Networking-Group-Bump #185: httpclient5 ≥ 5.6.3) | Historische Begründung (11.09.): Ktor-3.x-Buildzeitlinie (ktor-client-apache5 3.5.2, nicht im Vivid-Graph); Pool-Exhaustion-Szenario betrifft Langzeit-Server-Verbindungen. |

**Gemeinsamer Re-Assessment-Trigger:** Dependabot öffnet automatisch neue Alerts, sobald eine betroffene Dependency die verwundbare Range verlässt — die Dismissals beziehen sich immer nur auf die notierte Range. Enthält eine neue Dependency-Version weiterhin die Range, erscheint ein neuer Alert, der neu bewertet wird. **Lehre (empirisch, korrigiert 25.09.):** Dismissed Alerts können bei Fix **doch** automatisch auf `fixed` wechseln — #19/#20 (20.09.) und #62/#64/#65 (21.09.) wechselten nach der Graph-Resubmission selbstständig auf `fixed`. Die Beobachtung vom 11.09. („#26 bleibt dismissed, obwohl der Graph ≥ Fix auflöst") trifft damit nur noch auf #26 zu — den bislang einzigen Ausnahmefall (möglicherweise, weil der Fix dort bereits zur Dismissal-Zeit im Graph lag). Für das Register zählt deshalb weiterhin nicht primär der API-Zustand, sondern die dokumentierte Ist-Version: sie ist die Wahrheit über die Befund-Substanz; der monatliche Review-Workflow gleicht den Zustand ab.
...
**Manuelle SonarCloud-UI-Aktion (kein Token/API-Dismissal):** Der Security-Hotspot `xml:S5332` („usesCleartextTraffic implicitly enabled for older Android versions", `network_security_config.xml`) lässt sich nur in der SonarCloud-Oberfläche auf **„SAFE / Abgedeckt"** reviewen — sonar-scanner/Token-basiert ist kein Dismissal möglich. Grund ist in SECURITY.md „Netzwerk-Sicherheit (Cleartext blockiert)" dokumentiert; ist der Hotspot in der UI als SAFE markiert, wird er von SonarCloud nicht mehr als neu gewertet.
...
| Datum | Änderung |
|---|---|
| 2026-09-25 | **Dependabot-Sektion aktualisiert:** Live-Abgleich der 9 Dismissals — 5 wurden durch Fixes automatisch geschlossen (#19/#20 am 20.09. via BC-Buildscript-Pin 1.85, `892451c`; #62/#64/#65 am 21.09. via Networking-Group-Bump #185), dismissed verbleiben #14/#16/#26/#27 (davon #26 obsolet, Graph ≥ Fix). Lehre #26 korrigiert: Dismissed→fixed-Wechsel ist empirisch belegt (Gegenbeobachtung zur 11.09.-These); Quellentabelle (4 statt 9) und Statuszeilen nachgezogen. Zusätzlich #477/#478 Code-Scanning-Dismissals registriert (Kotlin-Temp-Artefakte, false positive) — Code Scanning 0 offen. |
| 2026-09-13 | **Re-Scan-Verifikation:** `#471 backup-enabled`, `#473 local-variable-is-never-read` und der **ursprüngliche `#474 deprecated-call`** wurden per Code-Fix analyseseitig als `fixed` geschlossen (der async-`GeocodeListener`-Pfad ab API 33 greift; die Query akzeptiert den SDK-Guard). Für den verbleibenden minSdk-24-Sync-Fallback erzeugte der Scan die **neue Instanz #476** (Zeile 75) — diese wurde per GitHub-API als `won't fix` dismissed (Register-Zeile #474 → #476 umgetitelt). `#472` persistierte trotz Source-Refactor (`!is`-Guard) als Kotlin-Temp-Finding (Dateiebene) und wurde am selben Tag als `false positive` gedismissed. **Repos-level Findings #23 (`BranchProtectionID`) und #44 (`CodeReviewID`/Scorecard)** als `won't fix` dismissed — Solo-Projekt: Codeowners-/Multi-Reviewer-Regeln und 30 fremde Approvals in 90 Tagen sind strukturell nicht erfüllbar; Teilhärtung (linear_history, Required Checks, 1 Review) bleibt aktiv. Damit sind **alle CodeQL-Alerts geschlossen** (0 offen). SonarCloud-Hinweis ergänzt: `xml:S5332`-Hotspot ist nur per **manueller UI-Aktion** als SAFE reviewbar (kein Token-/API-Dismissal). Register-Guard-Selbsttest um die beiden G7-Richtungen erweitert (T11: dismissed ohne Registereintrag → rot, T12: dismissed im Register → grün, offline per Stub-gh). |
