...
- **Shared-Chat-Bewusstsein:** Der Reader abonniert zusätzlich die drei EventSub-Topics `channel.shared_chat.begin`/`update`/`end` (v1, kein Scope nötig). Solange eine Twitch-Shared-Chat-Session („Stream Together“) läuft, zeigt das Chat-Overlay einen dezenten Hinweis (als Gast mit dem Session-Host, als Host mit der Kanal-Anzahl); bei Session-Ende/Stopp/Kanalwechsel verschwindet er. Der Owner-Gate-Schutz gegen Ko-Streamer-Badges bleibt davon unberührt (siehe Abschnitt 2).
