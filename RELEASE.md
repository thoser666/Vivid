...
> 🛡️ **Security-Lage (Stand 26.08.2026):** **Dependabot: 0 offene Alerts** — die 49 build-tooling-Alerts (1 critical, 21 high, ausschließlich Gradle/Fastlane/JVM-Abhängigkeiten ohne App-Runtime-Pfad, u. a. kotlin-gradle-plugin) wurden mit dokumentierter Begründung dismissiert (fix=KEIN; Tracking: Kotlin-2.4.20-stabil-Update im September, siehe Abschnitt „Ausstehend“ oben). **CodeQL: neu aktiv (oben)** — schließt die Lücke „echter Security-Scan“, da Dependabot nur Bibliotheken und DeepSource nur Stil/Komplexität abdeckt. **Optional (Post-Play-Release):** Sentry↔GitHub-Integration (app.sentry.io → Settings → Integrations → GitHub) verknüpft Crash-Issues mit „suspect commits“ (welcher Commit einen Crash ausgelöst hat) — erst sinnvoll, wenn echte Releases via Play laufen; kein Code-Aufwand, nur 1× im Dashboard verknüpfen.
