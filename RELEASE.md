...
>
> 🔗 **Markdown-Anker-Check:** [scripts/check_markdown_anchors.sh](scripts/check_markdown_anchors.sh) validiert bei jedem Push **alle internen Markdown-Anker** — Links auf eine andere Datei (Ziel.md#Anker) und Sprungmarken innerhalb derselben Datei (#Anker) — deterministisch mit dem **GitHub-Anker-Algorithmus** (empirisch gegen gerenderte Seiten verifiziert). Tote Anker nach Abschnitts-Umbenennungen und kaputte relative Pfade (z. B. aus `docs/` heraus `../RELEASE.md` statt `RELEASE.md`) brechen die CI statt still zu 404en. Externe URLs und GitHub-UI-Links (z. B. `issues/new?template=…`) werden übersprungen. Selbsttest ([scripts/test_markdown_anchors.sh](scripts/test_markdown_anchors.sh), 3 Positiv- + 5 Negativfälle: toter Anker same-file/cross-file, fehlende Ziel-Datei (auch Nicht-md wie Bilder/SVGs), Umlaut-/Variation-Selector-Header) läuft ebenfalls in android.yml; lokal: `bash scripts/check_markdown_anchors.sh`.
