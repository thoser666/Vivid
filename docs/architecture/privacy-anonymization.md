...
> **Status:** Skizze festgelegt (2026-09-26) · **Umsetzung: nicht begonnen** — Start mit P0 (Filter-Komposition, verhaltensneutral) · **Datum:** 2026-09-26
...
| Phase | Inhalt | Proof |
|---|---|---|
| **P0** | **Filter-Komposition (§4) + Privacy-Shader-Schnittstelle:** `PrivacyBlurFilterRender` (N Ellipsen-Uniforms, weiche Kante, Pixelate-Modus), `PrivacyComposer` mit kanonischem Rebuild, Umstellung der drei bestehenden Controller auf Composer-Applier. Noch **kein** UI/Settings — die Ellipsen-Uniforms werden nur aus Tests/Debug befüttert. | Bestands-Filter-Suiten grün (ggf. umgeschrieben auf Composer-Semantik), Robolectric-GL-Test für den Render (Muster `HaldClutLutBitmapRobolectricTest`), Composer-Test (Kombination/Deaktivierung/Rebuild-Ordnung), App kompiliert, Pre-Push-Gate grün |
