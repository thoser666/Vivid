...
| Zusatz-Features (über Parität) | 1 | 1 | 1 | 3 |
| **Gesamt** | **19** | **5** | **21** | **45**† |
...
| I18n (lokalisierte Strings) | 🚧 | `feature-*` (eigene `res/values`) | `strings.xml`-Grundgerüst (`values/` + partiell `values-fr/`) vorhanden, aber die Compose-UI nutzt noch **≈74 hartkodierte deutsche Literale** (`Text("…")` in feature-settings/feature-obs-control/app/feature-streaming + ~20 weitere Muster) — konkreter Modul-für-Modul-Plan: [docs/i18n-plan.md](docs/i18n-plan.md); danach `values-en` vollständig + CI-Gates (Externalisierungs- + Vollständigkeits-Check) |

## 💡 Zusatz-Features (über Moblin-Parität hinaus)

| Feature | Status | Modul | Offene Tasks / Notizen |
|---------|--------|-------|------------------------|
| UI-Farbschemata (Dark/Light folgt dem System, Vivid-Palette) | 🚧 | `app` (`ui/theme/Theme.kt`) | **Stufe 1 fertig (18.08.):** Vivid-Grün-Palette (Material-3, Seed `#3DDC84`) ersetzt die Template-Farben (Purple/Pink) und `VividTheme` folgt jetzt dem System (`isSystemInDarkTheme()`) statt hartkodiert Light. **Stufe 2 offen:** User-Toggle (System/Hell/Dunkel/AMOLED) + optionale Akzentfarbe als neue Settings-Kategorie „Darstellung“. Abgrenzung: **nicht** zu verwechseln mit dem Moblin-Paritäts-Feature „Color-Spaces (sRGB/P3/Log) + 3D-LUTs“ (Video-Farbraum, Zeile oben — eigenes Roadmap-Bucket) |
