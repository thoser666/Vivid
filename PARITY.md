...
| Moblin-Feature | Status | Modul | Offene Tasks / Notizen |
|----------------|--------|-------|------------------------|
| Plattform-Chat (Twitch, Kick, YouTube, SOOP) | ✅ Twitch-Scope | `feature-chat` | **Twitch vollständig** (`TwitchChatClient`, `SocketIrcConnection`): anonym (justinfan) + TLS 6697, CAP (tags/commands/membership), JOIN/PING-PONG, IRCv3-Tags-Parser (display-name, color, badges, emotes, tmi-sent-ts, mod/sub), `state`-StateFlow + `messages`-Flow, Auto-Reconnect mit Backoff, Hilt-DI (`@ChatScope`); 14 Unit-Tests. **UI/Overlay ✓** (`ChatOverlay` + `ChatOverlayViewModel`), **KI-Chat-Bot ✓** (Modus-Switch, `!tts`, Media, Koexistenz, Begrenzungen). **Scope-Entscheidung (Beta-Gate):** „Twitch-Chat“ gilt als ✅ — Lesen (anonym), Overlay und Bot laufen. **Post-Beta-Roadmap:** Kick (WebSocket), YouTube (innertube), SOOP; OAuth-Login für Senden/Moderation |
