...
> **CodeQL-Alert-Bestand (Stand 28.08.2026):** **0 Errors** (1 False Positive: `implicit-pendingintents` dismissiert — Code nutzt explizite Intents + `FLAG_IMMUTABLE`, siehe github.com/github/codeql/issues/20153); **0 Warnings** (2x `field-masks-super-field` dismissiert — Kotlin interne $stable-Felder in Data-Klasses, bekannter False Positive); **7 Notes** — `java/local-variable-is-never-read` (6× unbenutzte lokale Variablen) + `java/android/backup-enabled` (Application Backup erlaubt). **10 Alerts fixed** — 6x `actions/missing-workflow-permissions`, 2x `field-masks-super-field` (False Positive), 2x `override val message` (präventiv). Insgesamt: 7 offen (0 Error, 0 Warning, 7 Note), 3 False Positives.
...
> - **CodeQL (oben aktiviert):** Scan läuft sauber (`success`), 7 offene Alerts (0 Error: `implicit-pendingintents` als False Positive dismissiert, 0 Warning: `field-masks-super-field` als False Positive dismissiert — Kotlin interne $stable-Felder, 7 Note: `local-variable-is-never-read`/`backup-enabled`), 10 fixed, 3 False Positives. **Default Setup deaktiviert** (blockierte SARIF-Uploads).
