...
| `!start` / `!go-live` · `!stop` / `!end` · `!diag` / `!status` · `!ask <frage>` · `!fix` · `!testalert <follow\|sub\|gift\|resub\|raid>` | **nur der Streamer** (Broadcaster-Badge oder Allow-List `chat_bot_owner_logins`) | Stream starten/stoppen · Diagnose mit Empfehlungen · Auto-Fix · Frage an die **exklusive Owner-KI** (Fallback: normale Bot-KI) · Test-Alert fürs Chat-Overlay — nur während eines aktiven Streams; Viewer erhalten nur einen Hinweis |
...
> - **CodeQL (oben aktiviert):** Scan läuft sauber (`success`), 9 offene Alerts (0 Error: `implicit-pendingintents` als False Positive dismissiert, 2 Warning: `field-masks-super-field`, 7 Note: `local-variable-is-never-read`/`backup-enabled`), 6 fixed, 1 False Positive. **Default Setup deaktiviert** (blockierte SARIF-Uploads). False Positive: Code nutzt explizite Intents + `FLAG_IMMUTABLE`.
